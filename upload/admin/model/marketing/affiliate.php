...
class ModelMarketingAffiliate extends Model {
	...
	public function editAffiliate($affiliate_id, $data) {
		...
		if ($data['password']) {
			$this->db->query("UPDATE " . DB_PREFIX . "affiliate SET password = '" . $this->db->escape(password_hash(html_entity_decode($data['password'], ENT_QUOTES, 'UTF-8'), PASSWORD_DEFAULT)) . "' WHERE affiliate_id = '" . (int)$affiliate_id . "'");
		}
	}
